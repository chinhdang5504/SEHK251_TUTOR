...
import { Search, Bell, BellOff, ChevronDown } from 'lucide-react'
...
const HeaderSearch = ({ onSearch, placeholder = 'Search tutor by name, subject, department' }: HeaderProps) => {
  ...
  return (
    <div className='fixed top-0 left-0 right-0 z-50 bg-white backdrop-blur-md flex items-center h-16 px-5'>
      ...
      <div className='flex-1 relative mx-[80px] flex items-center'>
        ...
        />
        <Search
          className={`absolute left-3 top-1/2 -translate-y-1/2 transition-colors cursor-pointer ${searchText || (inputRef.current && inputRef.current === document.activeElement)
            ? 'text-red-500'
            : 'text-gray-400'
            }`}
      ...
      <div className='flex items-center gap-4 ml-6'>
        ...
        >
          {notiOn ? <Bell className='text-red-500' size={18} /> : <BellOff className='text-gray-700' size={18} />}
        </button>
        ...
        <div className='relative' ref={boardRef}>
          ...
          >
            ...
            <span className='text-gray-800 text-sm font-medium'>{user?.fullName || 'Guest'}</span>
