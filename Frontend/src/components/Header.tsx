...
const Header = () => {
    const getNavLinkClass = ({ isActive }: { isActive: boolean }) =>
        `px-4 py-2 rounded-lg transition-colors ${isActive ? 'bg-gray-300 text-gray-900' : 'text-gray-600 hover:bg-gray-200'
        }`
        ...
        <header className='w-full bg-white shadow-md sticky top-0 z-50'>
            <nav className='container mx-auto px-4 h-20 flex justify-between items-center'>
                <NavLink to='/' className='flex items-center gap-2'>
                    <img src={logoImg} alt='HCMUT Tutor Logo' className='h-15' />
                </NavLink>
